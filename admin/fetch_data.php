...
$sqlCount = "SELECT COUNT(*) AS nb_entries FROM $table WHERE Immatriculation LIKE ? OR Date_mise_en_service LIKE ?;";
$stmtCount = mysqli_prepare($connection, $sqlCount);
$searchParam = "%" . $searchQuery . "%";
mysqli_stmt_bind_param($stmtCount, 'sss', $searchParam, $searchParam);
...
$fetch_query = "SELECT * FROM $table WHERE Immatriculation LIKE ? ORDER BY id LIMIT ?, ?;";
$stmt = mysqli_prepare($connection, $fetch_query);
mysqli_stmt_bind_param($stmt, 'ssii', $searchParam, $first, $parPage);
